# 样本动态分析工具 (Malware Analysis Platform)

> 本地优先的 Windows 恶意软件分析沙箱 · 作者: xexingexe · v3.4.0

样本动态分析工具是一款 Windows 平台的本地恶意软件分析沙箱，支持静态分析、动态行为分析、网络流量捕获、多引擎威胁情报、内存取证与可视化报告，用于判断可疑文件（exe/dll/脚本/压缩包/Office 文档/URL）是否恶意。
...
### 方式一：打包版（推荐，免装环境）
1. 运行 `样本动态分析工具.exe`（onedir 打包，含全部依赖与 ClamAV 引擎）
